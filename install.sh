...
echo 'alias exp="$HOME/core-bridgechain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-bridgechain"
git clone "https://github.com/munich/5.git" "$HOME/core-bridgechain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/munich/5.git'"
...
rm -rf "$HOME/.config/@exp"
rm -rf "$HOME/.config/@examplechain"
rm -rf "$HOME/.config/exp-core"
